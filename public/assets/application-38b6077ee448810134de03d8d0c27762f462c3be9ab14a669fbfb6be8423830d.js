...
(function() {
  var RichMarkerBuilder, check_in_bound, deal_coordinate, rerender_map,
    extend = function(child, parent) { for (var key in parent) { if (hasProp.call(parent, key)) child[key] = parent[key]; } function ctor() { this.constructor = child; } ctor.prototype = parent.prototype; child.prototype = new ctor(); child.__super__ = parent.prototype; return child; },
    hasProp = {}.hasOwnProperty;
  ...
  RichMarkerBuilder = (function(superClass) {
    ...
    RichMarkerBuilder.prototype.create_infowindow = function() {
      ...
      boxText.setAttribute('class', 'marker_container gmap_card');
      boxText.innerHTML = this.args.html;
    ...
    RichMarkerBuilder.prototype.infobox = function(boxText) {
      return {
        ...
        closeBoxURL: this.args.close_url,
  ...
  this.buildMap = function(markers) {
    ...
    return handler.buildMap({
      ...
      var prevent_prerender;
      markers = handler.addMarkers(markers);
      handler.bounds.extendWith(markers);
      handler.fitMapToBounds();
      prevent_prerender = false;
      return google.maps.event.addListener(handler.getMap(), 'idle', function() {
        var coordinate, ne, show_card_num, sw;
        ...
        return prevent_prerender = true;
      });
    });
  };

  check_in_bound = function(coordinate) {
    var a, b;
  ...
  deal_coordinate = function(ne, sw) {
    var e, n, s, w;
  ...
  rerender_map = function(coordinate) {
    var qty;
    qty = 9;
    if ($(window).width() < 992) {
      qty = 8;
...
(function() {
  $(document).ready(function() {
    return $(document).on('click', '.bigbun.after_prepartion', function(e) {
      var id;
      e.preventDefault();
      if (location.pathname === '/') {
        id = $(this).closest('.ui.card').data('id');
        return $('#bigbun_modal_' + id).modal('show');
      } else if (location.pathname.match(/(restaurants\/\d+)/gi)) {
        return $('.bigbun_modal').modal('show');
      }
    });
  });
...
(function() {
  $(document).ready(function() {
    ...
    $('.sign_up').click(function() {
      return $('#sign_up_modal').modal('show');
    });
    $('.login').click(function() {
      return $('#login_modal').modal('show');
    });
    return $(document).on('click', '.navbar-collapse.in', function(e) {
      if ($(e.target).is('a')) {
...
(function() {
  $(document).ready(function() {
    var deal_bigbun_list, deal_food_list, get_cart_list, render_food_qty, render_order_new_list, render_total_price, save_in_cookie;
    get_cart_list = function() {
      if ($.cookie('cart_list') === void 0) {
        return {};
      } else {
        return $.parseJSON($.cookie('cart_list'));
      }
    };
    save_in_cookie = function(data) {
      return $.cookie('cart_list', JSON.stringify(data), {
        expires: 1,
        path: '/'
      });
    };
    render_food_qty = function() {
      var list;
      list = get_cart_list();
      return Object.keys(list).forEach(function(restaurant, i, a) {
        return Object.keys(list["" + restaurant]).forEach(function(food, i, a) {
          var id, qty;
          if (food.indexOf('food') !== -1) {
            qty = +list["" + restaurant]["" + food]['qty'];
            id = +food.replace('food_', '');
            return $("[data-food-id='" + id + "']").find('#txtNum').val(qty);
          }
        });
      });
    };
    deal_bigbun_list = function(_this) {
      var bigbun_code, bigbun_id, info_object, modal, obj, restaurant_id;
      ...
      if (info_object["restaurant_" + restaurant_id]) {
        ...
        info_object["restaurant_" + restaurant_id] = (
          obj = {},
          obj["bigbun_" + bigbun_id] = bigbun_code,
          obj
        );
      }
      return save_in_cookie(info_object);
    };
    deal_food_list = function(_this) {
      var _data, food_id, food_price, img, img_url, info_object, name, obj, qty, restaurant_cost, restaurant_distance, restaurant_id, restaurant_tax;
    ...
    render_order_new_list = function() {
      var cart_list, product_list;
      product_list = $('.product_list').first();
      cart_list = get_cart_list();
      return Object.keys(cart_list).forEach(function(restaurant, i, a) {
        return Object.keys(cart_list["" + restaurant]).forEach(function(food, i, a) {
          var food_id, img_url, name, price, qty, restaurant_id, restaurant_tax, this_list;
          if (food.indexOf('food') !== -1) {
            if (i !== 0) {
            ...
            return this_list.find('.food_data_info').attr('data-food-name', name).attr('data-food-id', food_id).attr('data-food-price', price).attr('data-restaurant-id', restaurant_id).attr('data-restaurant-tax', restaurant_tax);
          }
        });
      });
    };
    render_total_price = function() {
      var cart_list, food_price, tax_price, tip, total_price;
      cart_list = get_cart_list();
      tax_price = 0;
      food_price = 0;
      Object.keys(cart_list).forEach(function(restaurant, i, a) {
        var restaurant_tax;
        restaurant_tax = +cart_list["" + restaurant]['tax'];
        return Object.keys(cart_list["" + restaurant]).forEach(function(food, i, a) {
          var price, qty;
      ...
      return $('[name=cart_total_price]').html(food_price.toFixed(2));
    };
    render_food_qty();
    render_total_price();
    $(document).on('click', ".product_spineer_button", function() {
      deal_food_list(this);
      render_total_price();
      if (location.pathname === "/orders/new") {
        return render_total_price();
      }
    });
    $(document).on('click', ".bigbun_modal #redeem_now", function() {
      deal_bigbun_list(this);
      return $('.bigbun_modal').modal('hide');
    });
    if (location.pathname === "/orders/new") {
      render_order_new_list();
      render_total_price();
      $('input.tip_input').change(function() {
        return render_total_price();
      });
      return $('.orders_new .ui.form form').submit(function(e) {
        var cart_list, data;
        e.preventDefault();
        ...
        Object.keys(cart_list).forEach(function(restaurant, i, a) {
          var restaurant_id;
          restaurant_id = +restaurant.replace('restaurant_', '');
          return Object.keys(cart_list["" + restaurant]).forEach(function(_key, i, a) {
            var bigbun_id, code, food_id, input, qty;
            if (_key.indexOf('food') !== -1) {
              ...
              return $('.orders_new .ui.form form').append($(input));
